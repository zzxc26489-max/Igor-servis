...
export function issuedVehicleOrders(orders: Order[], vehicleId: string) {
  return vehicleOrders(orders, vehicleId).filter((order) => order.status === "выдан");
}

export function vehicleServiceStats(orders: Order[], vehicleId: string) {
  const history = vehicleOrders(orders, vehicleId);
  const issued = issuedVehicleOrders(orders, vehicleId);
