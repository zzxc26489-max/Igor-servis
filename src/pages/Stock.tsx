      ...
      <Page>
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.45fr_.86fr_.64fr]">
          <Card className="p-0 overflow-hidden">
            <div className="p-5 border-b" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button className="rounded-lg border px-3 hover:bg-gray-50" style={{ borderColor: "var(--border)" }} aria-label="Сканировать камерой" onClick={() => setNotice("Камера будет подключена после запуска серверной версии CRM.")}><IconCamera size={22} /></button>
              </div>
            ...
            <div className="max-h-64 overflow-auto divide-y" style={{ borderColor: "var(--border)" }}>
              {shownStock.map((item) => (
                <button key={item.id} className="flex w-full items-center gap-3 p-4 text-left hover:bg-[#f7faf8]" onClick={() => { setSelectedId(item.id); setCell(item.cell || "A-03-02"); setNotice(""); }}>
          ...
          <Card>
            ...
            <div className="mt-5 grid grid-cols-4 gap-1 text-center text-[10px]">{CELLS.map((value) => <button onClick={() => setCell(value)} key={value} className="rounded border px-1 py-2" style={{ borderColor: value === cell ? "var(--accent)" : "var(--border)", background: value === cell ? "var(--accent)" : occupiedCells.has(value) ? "#f1f3f2" : "white", color: value === cell ? "white" : "var(--text)" }}>{value}</button>)}</div>
          </Card>
