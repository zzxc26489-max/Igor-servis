  ...
  return <>
    ...
    <Page>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.35fr_.8fr]">
        ...
        <Card className="overflow-hidden p-0"><div className="flex items-center justify-between border-b p-4" style={{ borderColor: "var(--border)" }}><div><p className="section-kicker">Команда</p><h2 className="panel-title mt-1">Зарплата сотрудников</h2></div><span className="text-xs text-[var(--text-muted)]">По выполненным работам</span></div><div className="divide-y" style={{ borderColor: "var(--border)" }}>{employees.map((employee) => <div key={employee.id} className="p-4"><div className="flex items-center justify-between gap-3"><span className="flex min-w-0 items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#e9f4ed] text-xs font-bold text-[var(--accent)]">{employee.name.slice(0,2).toUpperCase()}</span><span className="min-w-0"><b className="block truncate text-sm">{employee.name}</b><span className="muted block text-xs">{employee.role}</span></span></span><b className="whitespace-nowrap text-sm tabular-nums">{formatMoney(employee.accrued)}</b></div><p className="muted mt-2 pl-10 text-xs">{employee.payType === "percent" ? `${employee.payValue}% от работ` : employee.payType === "salary" ? "Оклад" : "Оклад + %"}</p></div>)}</div><div className="border-t bg-[#fafbfa] p-4 text-right font-semibold" style={{ borderColor: "var(--border)" }}>Итого к выплате: {formatMoney(totalSalaries)}</div></Card>
      </div>
